...
if len(sys.argv) < 2:
    ...
    print("Correct usage:")
    print("$ python3 simple_plot.py <path>")
    sys.exit()
...
whole_file_input = input('Use whole file? [y/n] ')
if whole_file_input == 'y':
    time_based = True
    start_time = 0
    end_time = 181
else:
    time_based = True
    start_time_input = input('Enter start time [s]: ')
    start_time = float(start_time_input)

    end_time_input = input('Enter end time [s]: ')
