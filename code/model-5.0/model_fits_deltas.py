# model_fits_deltas.py
#
# Fits MAP data to a model with two fit paramters: delta1 and delta2
#
# Created: 2025.12.20
# Lexie Scholtz
...
num_guesses = 100
d1_guesses = -1 * np.logspace(-6, -1, num=num_guesses)
d2_guesses = -1 * np.logspace(1, 9, num=num_guesses)
guesses = []
for d1 in d1_guesses:
    for d2 in d2_guesses:
        guesses.append([d1, d2])
...
def deltas_model(t, delta1, delta2):
...
for f in to_run_list:
    ...
    file_lines.append('Fit model: deltas_model')
    file_lines.append('Fit params: delta1, delta2')
    ...
    file_lines.append('\tdelta1: [{:0.4e}, {:0.4e}] ({} values with log spacing)'.format(d1_guesses[0], d1_guesses[-1], num_guesses))
    file_lines.append('\tdelta2: [{:0.4e}, {:0.4e}] ({} values with log spacing)'.format(d2_guesses[0], d2_guesses[-1], num_guesses))
    file_lines.append('Fit bounds:')
    file_lines.append('\tdelta1: [{:0.4e}, {:0.4e}]'.format(bounds[0][0], bounds[1][0]))
    file_lines.append('\tdelta2: [{:0.4e}, {:0.4e}]'.format(bounds[0][1], bounds[1][1]))
    ...
    for init_guess in tqdm(guesses):
        try:
            (popt, pcov) = curve_fit(deltas_model, time_shifted, conc_shifted, p0=init_guess, bounds=bounds)
            adj_model_y = deltas_model(time_shifted, *popt)
            ...
            chi = (delta1 * delta2 * rho_p * mu0 * (1+chi_s)) / (2 * (a**2))
            r = ( ((delta1 + delta2) * 2 * rho_p) / (-9) ) ** (1/2)
            ...
            for key in update_keys:
                best_fits[key]['guess_d1'] = init_guess[0]
                best_fits[key]['guess_d2'] = init_guess[1]
                best_fits[key]['d1'] = popt[0]
                best_fits[key]['d2'] = popt[1]
                ...
                best_fits[key]['se_d1'] = se_d1
                best_fits[key]['se_d2'] = se_d2
            ...
            print_str += ('\t{chi:0.8e}\t{r:0.8e}\t{mse:0.8e}\t{r_sq:0.12f}\t{guess_d1:0.8e}'
                       '\t{guess_d2:0.8e}\t{d1:0.8e}\t{d2:0.8e}\t{cov_aa:0.8e}\t'
                       '\t{cov_ab:0.8e}\t{cov_ba:0.8e}\t{cov_bb:0.8e}'
                       '\t{se_d1:0.8e}\t{se_d2:0.8e}\t{corr:0.8e}').format(guess_d1=init_guess[0],
                guess_d2=init_guess[1], d1=popt[0], d2=popt[1], chi=chi, r=r,
                cov_aa=cov_00, cov_ab=cov_01, cov_ba=cov_10, cov_bb=cov_11,
                se_d1=se_d1, se_d2=se_d2, corr=corr, mse=mse, r_sq=r_sq)
    ...
    print("\nTWO-PHASE MODEL")
    print("file: " + str(file))
    print('error_metric\tchi\tr\tmse\tr_sq\tguess_d1\tguess_d2\td1\td2\tcov_00'
        + '\tcov_01\tcov_10\tcov_11\tse_d1\tse_d2\tcorr')
    for key in best_fits:
        d = best_fits[key]
        print('{metric}\t{chi:0.8e}\t{r:0.8e}\t{mse:0.8e}\t{r_sq:0.12f}\t{guess_d1:0.8e}\t{guess_d2:0.8e}\t{d1:0.8e}'.format(
                metric=key, chi=d['chi'], r=d['r'], mse=d['mse'], r_sq=d['r_sq'], guess_d1=d['guess_d1'], guess_d2=d['guess_d2'], d1=d['d1'])
            + '\t{d2:0.8e}\t{cov_aa:0.8e}\t'.format(
                d2=d['d2'], cov_aa=d['cov_00'])
            + '\t{cov_ab:0.8e}\t{cov_ba:0.8e}\t{cov_bb:0.8e}'.format(
                cov_ab=d['cov_01'], cov_ba=d['cov_10'], cov_bb=d['cov_11'])
            + '\t{se_d1:0.8e}\t{se_d2:0.8e}\t{corr:0.8e}'.format(
                se_d1=d['se_d1'], se_d2=d['se_d2'], corr=d['corr'])
            + ''.format())
    ...
    file_lines.append('\nFINAL RESULTS')
    file_lines.append('error_metric\tchi\tr\tguess_d1\tguess_d2\td1\td2\tcov_00'
        + '\tcov_01\tcov_10\tcov_11\tse_d1\tse_d2\tcorr\tmse\tr_sq')
    for key in best_fits:
        d = best_fits[key]
        file_lines.append(('{metric}\t{chi:0.8e}\t{r:0.8e}\t{mse:0.8e}\t{r_sq:0.12f}\t{guess_d1:0.8e}'
            '\t{guess_d2:0.8e}\t{d1:0.8e}\t{d2:0.8e}\t{cov_aa:0.8e}\t'
            '\t{cov_ab:0.8e}\t{cov_ba:0.8e}\t{cov_bb:0.8e}'
            '\t{se_d1:0.8e}\t{se_d2:0.8e}\t{corr:0.8e}'
            '').format(metric=key,
                guess_d1=d['guess_d1'], guess_d2=d['guess_d2'], d1=d['d1'],
                d2=d['d2'], chi=d['chi'], r=d['r'], cov_aa=d['cov_00'],
                cov_ab=d['cov_01'], cov_ba=d['cov_10'], cov_bb=d['cov_11'],
                se_d1=d['se_d1'], se_d2=d['se_d2'], corr=d['corr'],
    ...
    for key in best_fits:
        ...
        file_lines.append('error_metric\tchi\tr\tmse\tr_sq\tguess_d1\tguess_d2\td1\td2\tcov_00'
            + '\tcov_01\tcov_10\tcov_11\tse_d1\tse_d2\tcorr')
    ...
    for key in best_fits:
        d = best_fits[key]
        ax3.plot(time_shifted, deltas_model(time_shifted, d['d1'], d['d2']),
