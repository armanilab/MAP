...
try:
    (popt, pcov) = curve_fit(working_model, time_shifted, conc_shifted, bounds=bounds)
except ValueError:
    pass
except RuntimeError:
    pass
...
### SAVE RESULTS
if to_save:
    with open(save_dir + file[:-4] + '-noguesses-fit' + file_suffix + '.txt', 'w') as f:
        for l in file_lines:
            f.write(l + '\n')

    plt.savefig(save_dir + file[:-4] + '-noguesses-fit' + file_suffix + '.png', dpi=600)
    print('Saved ' + save_dir + file[:-4] + '-no-guessesfit' + file_suffix + ' as .txt and .png.')
else:
    plt.show()
