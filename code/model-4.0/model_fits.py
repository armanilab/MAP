...
chis = np.logspace(-5, 2, num=100)
rs = np.logspace(-9, -4, num=100)
guesses = []
...
file_lines.append('\tChi: [{:0.2e}, {:0.2e}] (100 values with log spacing)'.format(chis[0], chis[-1]))
file_lines.append('\tRadius: [{:0.6e}, {:0.6e} (100 values with log spacing)]'.format(rs[0], rs[-1]))
file_lines.append('Bounds: ')
file_lines.append('\tChi: [{:0.2e}, {:0.2e}]'.format(bounds[0][0], bounds[0][1]))
file_lines.append('\tRadius: [{:0.2e}, {:0.2e}]'.format(bounds[1][0], bounds[1][1]))
...
ax3.plot(time_shifted + time[global_min_index], best_r_sq_y, '--', color='blue', label='Best $r^2$ fit')
if optimize_radius:
